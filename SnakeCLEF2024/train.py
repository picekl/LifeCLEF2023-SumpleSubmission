...
def load_metadata(config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    BASE_IMAGE_PATH_iNAT = "./images_inat"
    BASE_IMAGE_PATH_HMP = "./images_hmp"

    train_metadata_iNat = pd.read_csv("./metadata/SnakeCLEF2023-TrainMetadata-HM.csv")
    ...
    metadata = pd.concat(
        [train_metadata_iNat, train_metadata_hmp, val_metadata]).reset_index(drop=True)

    train_df = metadata[metadata.subset != "val"]
    valid_df = metadata[metadata.subset == "val"]
